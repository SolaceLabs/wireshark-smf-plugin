    ...
    while( local_offset < offset+size )
    {
        ...
        if(flowid == 0xFFFFFFFF) {
            if(messageCount == 1 && lastMsgId == 0) {
                item = proto_tree_add_string_format(sub_tree,
                hf_assuredctrl_transactionflow_string, tvb, offset, 16, NULL, "ROLLBACK_ONLY_PUBLISHER");
            } else {
                item = proto_tree_add_string_format(sub_tree,
                hf_assuredctrl_transactionflow_string, tvb, offset, 16, NULL, "INVALID_FLOW_ID");
            }
            proto_item_set_generated(item);
        }
